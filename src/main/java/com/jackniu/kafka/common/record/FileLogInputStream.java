...
import com.jackniu.kafka.common.utils.CloseableIterator;
...
public class FileLogInputStream  implements LogInputStream<FileLogInputStream.FileChannelRecordBatch>{
    ...
    public FileChannelRecordBatch nextBatch() throws IOException {
        ...
        if (magic < RecordBatch.MAGIC_VALUE_V2)
            batch = new AbstractLegacyRecordBatch.LegacyFileChannelRecordBatch(offset, magic, fileRecords, position, size);
        else
            batch = new DefaultRecordBatch.DefaultFileChannelRecordBatch(offset, magic, fileRecords, position, size);
