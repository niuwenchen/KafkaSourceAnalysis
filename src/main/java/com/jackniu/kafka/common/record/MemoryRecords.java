...
public class MemoryRecords extends AbstractRecords {
     ...
     * @throws  if record size or magic is invalid
     */
    ...
    private static FilterResult filterTo(TopicPartition partition, Iterable<MutableRecordBatch> batches,
        ...
        for (MutableRecordBatch batch : batches) {
            long maxOffset = -1L;
            RecordFilter.BatchRetention batchRetention = filter.checkBatchRetention(batch);
            filterResult.bytesRead += batch.sizeInBytes();

            if (batchRetention == RecordFilter.BatchRetention.DELETE)
                continue;
