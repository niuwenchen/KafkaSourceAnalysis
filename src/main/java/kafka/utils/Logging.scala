...
object Log4jControllerRegistration{
  ...
  try{
    val log4jControllerRegistration = Class.forName("kafka.utils.Log4jController").asInstanceOf[Class[Object]]
    val instance = log4jControllerRegistration.getDeclaredConstructor().newInstance()
...
trait Logging {
  ...
  def isDebugEndabled:Boolean = logger.underlying.isDebugEnabled
